...
class CustomUserCreationForm(UserCreationForm):
	...
	class Meta(UserCreationForm):
		model = CustomUser 
		fields = ['username','first_name', 'last_name', 'email', 'phone_number', 'birth_date', 'organization_name', 'organization_address', 'sex', 'position']
...
class ContractForm(forms.ModelForm):
	...
	class Meta:	
		model = Contract
		exclude = ['issued_date', 'is_final', 'client', 'link', 'is_finished', 'is_approved', 'status']
...
class GetLocationForm(forms.Form):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['locations'].queryset = Location.objects.filter(owner=user, include = False, contract = None)
...
class QuickShiftTypeForm(forms.Form):
	...
	def clean(self):
		...
		if int(shifts) == 3 and int(hours) == 12:
			raise ValidationError('Total working hours (shifts * hours) should not exceed 24 hours')
	...
	def __init__(self, *args, **kwargs):
		...
		try:
			...
			for instruction in designation.instructions.all():
				...
				avoid_list = [x.id for x in Instruction.objects.filter(post = post, shift__day=day)]
		...
		self.fields['instruction'].queryset = Instruction.objects.exclude(id__in = same_day).filter(id__in = instruction_map)
...
class RemakeAddRelieverForm(forms.Form):
	...
	def __init__(self, *args, **kwargs):
		...
		self.fields['instruction'].queryset = Instruction.objects.filter(id__in = day_off_list)