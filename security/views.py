			...
			if proceed:
				for day in range(day_start, day_end + 1):
					...
					for i in range(number_of_shifts):
						...
						start = end 
						shift.save()
						instruction = Instruction.objects.create(shift = shift, post = post, number_of_guards = number_of_guards)
						instruction.save()
...
class DDOSearchSecurityView(View):
	def get(self, request, ddo_pk, pk, *args, **kwargs):
		...
		if fname and lname:
			qs = qs.filter(Q(first_name__startswith = fname) | Q(last_name__startswith = lname))
...
class DDOAssignSecurityView(View):
	...
	def post(self, request, ddo_pk, post_pk, pk, *args, **kwargs):
		...
		post = Post.objects.get(id=post_pk)
		guard = Guard.objects.get(id=pk)
		form = Remake2AddSecurityForm(request.POST, ddo_pk = ddo_pk, post_pk = post_pk, guard_pk = pk)
		...
		if form.is_valid():
			...
			instructions = Instruction.objects.filter(shift__start_time = start, post=post)
			...
			if len(instruction_map) != 0:
				if day_off == 0:
					...
					try:
						day_off = Instruction.objects.get(shift__start_time = start, shift__day = day_off, post=post)
						
						designation = Designation.objects.create(guard=guard, ddo = ddo, day_off = day_off)
					except:
...
class DDOAssignRelieverView(View):
	def get(self, request, ddo_pk, post_pk, pk, *args, **kwargs):
		...
		designation = None 
		try:
			designation = Designation.objects.get(ddo=ddo, guard=guard, designation_type = 2)
		except:
			pass
	...
	def post(self, request, ddo_pk, post_pk, pk, *args, **kwargs):
		...
		post = Post.objects.get(id=post_pk)
		guard = Guard.objects.get(id=pk)
		form = RemakeAddRelieverForm(request.POST, ddo_pk = ddo_pk, post_pk = post_pk, guard_pk = pk)
		...
		if form.is_valid():
			...
			if not designation:
				designation = Designation.objects.create(ddo=ddo, guard=guard, designation_type = 2)
...
class ContractView(View):
	def get(self, request, *args, **kwargs):
		...
		pending_list = Contract.objects.filter(client=request.user, is_finished = True).exclude(status = 3)
